...
const DockingZone: FC<DockingZoneProps> = ({ 
  ...
  // Get appropriate default sizes based on position
  ...
  const getPositionStyles = (): CSSProperties => {
    ...
    const dimensionStyle = isHorizontal
      ? { height: hasVisiblePanels && !isCollapsed ? `${size}px` : '0px' }
      : { width: hasVisiblePanels && !isCollapsed ? `${size}px` : '0px' };
  ...
  return (
    <div
      className={`docking-zone relative bg-background ${getPositionClasses()} ${hasVisiblePanels ? (isCollapsed ? '' : 'active') : 'hidden'} transition-all duration-200 ${className}`}
