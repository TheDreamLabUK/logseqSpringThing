# Project Documentation

Welcome to the project documentation. This documentation provides comprehensive details about the client and server components of the system.

## Table of Contents

### Core Documentation
- [Overview](overview/architecture.md)
- [API Documentation](api/index.md)
  - [REST API](api/rest.md)
  - [WebSocket API](api/websocket.md)
  - [Binary Protocol](api/binary-protocol.md)
- [Security Documentation](security/index.md)
  - [Authentication](security/authentication.md)

### Server Documentation
- [Server Documentation](server/index.md)
  - [Architecture](server/architecture.md)
  - [Actor System](server/actors.md)
  - [Handlers](server/handlers.md)
  - [Services](server/services.md)
  - [AI Services](server/ai-services.md)
  - [GPU Compute](server/gpu-compute.md)
  - [Feature Access](server/feature-access.md)
  - [Configuration](server/config.md)

### Client Documentation
- [Client Documentation](client/index.md)
  - [Architecture](client/architecture.md)
  - [Components](client/components.md)
  - [UI Components](client/ui-components.md)
  - [WebSocket Integration](client/websocket.md)
  - [XR Support](client/xr.md)

### Development & Deployment
- [Configuration Guide](configuration/index.md)
- [Development](development/index.md)
  - [Setup](development/setup.md)
  - [Testing](development/testing.md)
  - [Debugging](development/debugging.md)
- [Deployment](deployment/index.md)
  - [Docker](deployment/docker.md)
- [Contributing](contributing.md)

### Technical Deep Dives
- [Architecture](architecture/system-overview.md)
- [Decoupled Graph Architecture](technical/decoupled-graph-architecture.md)